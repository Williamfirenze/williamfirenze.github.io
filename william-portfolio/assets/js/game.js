/* =========================================================
   ROLLBACK — un gioco di William Firenze
   Un tasto: inverti la direzione.
   Prendi i COMMIT verdi, evita i LOCK rossi, raccogli il VACUUM.
   ========================================================= */
(function (global) {
  'use strict';
  ...
  Rollback.prototype.geo = function(){
    ...
    this.pr = clamp(this.R * 0.055, 6, 11);     // player radius
    this.cr = clamp(this.R * 0.048, 5, 9.5);    // commit radius
    this.lw = clamp(this.R * 0.085, 8, 16);     // lock stroke width
  };
  ...
  /* ---------- spawning ---------- */
  /* distanza in avanti, nel senso di marcia del giocatore */
  ...
  Rollback.prototype.freeAngle = function(minFromPlayer, minAhead){
    ...
    while (tries++ < 160) {
      ...
      for (i = 0; i < s.locks.length; i++) {
        if (angDist(a, s.locks[i].ang) < s.locks[i].half + 0.34) { bad = true; break; }
      }
      if (bad) continue;
      for (i = 0; i < s.commits.length; i++) {
        if (angDist(a, s.commits[i].ang) < 0.45) { bad = true; break; }
      }
      if (bad) continue;
      if (s.vac && angDist(a, s.vac.ang) < 0.45) continue;
      return a;
    }
    return norm(s.a + Math.PI);
  };

  Rollback.prototype.spawnCommit = function(){
    var C = { ang: this.freeAngle(0.85), born: this.s.t };
    this.s.commits.push(C);
    this.reachable(C, this.freeArc());
  };

  /* arco attualmente raggiungibile dal giocatore, in entrambi i sensi */
  ...
  /* se il giocatore resta incastrato, un lock viene rilasciato: mai morti ingiuste */
  ...
  /* tiene il commit dentro l'arco raggiungibile */
  Rollback.prototype.reachable = function(C, arc){
    var s = this.s;
    var cush = (this.pr + this.cr) / this.R + 0.06;
    var fwd = norm(C.ang - s.a), bwd = norm(s.a - C.ang);
    if (fwd <= Math.max(0, arc.cw - cush) || bwd <= Math.max(0, arc.ccw - cush)) return false;
    var room = Math.max(arc.cw, arc.ccw) - cush;
    if (room <= 0.1) return false;
    var sign = arc.cw >= arc.ccw ? 1 : -1;
    this.burst(this.ox + Math.cos(C.ang) * this.R, this.oy + Math.sin(C.ang) * this.R, 5, COL.okSoft, .6);
    C.ang = norm(s.a + sign * rand(room * 0.5, room * 0.92));
    return true;
  };

  Rollback.prototype.lockTarget = function(){
    if (this.s.t < 1.5) return 0;                       // respiro iniziale
    return Math.min(1 + Math.floor(this.s.score / 7), 6);
  };

  Rollback.prototype.spawnLock = function(instant){
    var s = this.s, lv = this.level();
    var half = clamp(0.135 + lv * 0.006, 0.135, 0.175);
    // keep at least one comfortable gap open
    var used = 0;
    for (var i = 0; i < s.locks.length; i++) used += s.locks[i].half * 2;
    if (used + half * 2 > 2.15) return;

    // mai addosso al giocatore: almeno ~1.15s di volo prima di arrivarci
    var a = this.freeAngle(1.15, clamp(s.spd * 1.15 + half, 1.35, 2.7));
    s.locks.push({
      ang: a,
      half: half,
      arm: instant ? 1 : 0,
      drift: (Math.random() < 0.5 ? -1 : 1) * rand(0.12, 0.30) * (1 + lv * 0.1),
      wob: rand(0, TAU)
    });
  };

  Rollback.prototype.spawnVac = function(){
    this.s.vac = { ang: this.freeAngle(1.2), life: 7.5, age: 0 };
  };
  ...
  Rollback.prototype.update = function(dt){
    ...
    /* speed ramp */
    s.spd = Math.min(1.75 + lv * 0.115, 3.5);
    ...
    for (i = 0; i < s.locks.length; i++) {
      var L = s.locks[i];
      if (L.arm < 1) L.arm = Math.min(1, L.arm + dt / 0.95);
      L.wob += dt;
      L.ang = norm(L.ang + L.drift * dt * (1 + Math.sin(L.wob * 0.7) * 0.25));
    }
    // top up locks
    while (s.locks.length < this.lockTarget()) {
      var before = s.locks.length;
      this.spawnLock(false);
      if (s.locks.length === before) break;
    }

    /* fairness: mai chiusi in una gabbia, mai un commit irraggiungibile */
    var arc = this.freeArc();
    if (arc.span < 0.62) { this.relief(); arc = this.freeArc(); }
    ...
    if (s.vac) {
      s.vac.age += dt;
      s.vac.life -= dt;
      if (angDist(s.a, s.vac.ang) < (this.pr + this.cr * 1.25) / this.R + 0.015) {
        this.vacuum();
      } else if (s.vac.life <= 0) {
        s.vac = null;
      }
    }

    /* collision with armed locks */
    for (i = 0; i < s.locks.length; i++) {
      var K = s.locks[i];
      if (K.arm < 1) continue;
      if (angDist(s.a, K.ang) < K.half + this.pr / this.R * 0.85) { this.die(K.ang); return; }
    }
  ...
  Rollback.prototype.draw = function(){
    ...
    for (i = s.trail.length - 1; i >= 1; i--) {
      ...
      if (angDist(a0, a1) > 0.6) continue; // wrap guard
      c.save();
